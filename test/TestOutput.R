...
## Load dataset 01 ---------------------------------------------------------------
fdata <- read.table("output_01.csv", sep = ",", header = TRUE, stringsAsFactors = TRUE)
...
ggplot(fdata) + geom_point(mapping = aes(x = PrevOffers, y = TotalGas, color = TicketCreation, size = Quantity, shape = PriceChange))
ggplot(fdata) + geom_point(mapping = aes(x = PrevOffers, y = TotalGas, color = TicketCreation, size = 2))

## Load dataset 02 ---------------------------------------------------------------
fdata <- read.table("output_02.csv", sep = ",", header = TRUE, stringsAsFactors = TRUE)
fdata$Period = as.factor(fdata$Period)
fdata$index = rownames(fdata)
plot(fdata$index, fdata$TotalGas, type = "overplotted", pch = 1, col = "green", ylim = c(20000, 600000), ylab = "Gas", xlab = "Balance Updates")
